...
if(empty($npsn)){
	...
	try{
		...
		$stmt = $db->prepare($sql);
		$stmt->execute(array($npsn, $nama, $alamat, $kelurahan, $kecamatan, $jenjang, $status, $waktu, $akreditasi, $sk_akreditasi, $tanggal_sk_akreditasi, $sk_pendirian, 					$tanggal_sk_pendirian, $sk_ijin, $tanggal_sk_ijin, $mbs, $sertifikat_iso, $akses_internet, $telpon, $fax, $email, $website, $rekening, $nama_bank, 					$cabang_bank, $lintang, $bujur));
		echo 'berhasil';
		}
		else{
			echo "<small class='text-red'><i  class='fa fa-info-circle'></i> Arsip dengan kode NPSN <b>".$npsn."</b> sudah ada!</small><br/>";
		}
