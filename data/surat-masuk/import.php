		...
		$FileType = pathinfo($target_file,PATHINFO_EXTENSION);
			...
			if (($handle = fopen($target_file, "r")) !== FALSE) {
					...
					while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
						...
						$cek= $db->prepare("select nomor_surat from e_surat_masuk where nomor_surat=? limit 1");
						$cek->execute(array($row[1]));
						if(empty($row[1])){
							exit;
						}
						else if(!$cek->rowCount()>0){
							$sql="INSERT INTO e_surat_masuk (tanggal, nomor_surat, perihal, pengirim, penerima, keterangan) VALUES (:tanggal, :nosurat, :perihal, :pengirim, :masuk, :penjelasan)";
							$stmt = $db->prepare($sql);
