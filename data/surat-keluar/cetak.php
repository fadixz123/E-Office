...
try {
			...
			$tanggal='';
            $query = $db->prepare("SELECT tanggal, nomor_surat, perihal, pengirim, penerima, keterangan FROM e_surat_keluar WHERE tanggal BETWEEN '$mulai' AND '$akhir' ");
			$query->execute();
			while($row = $query->fetch(PDO::FETCH_BOTH)) {
				$tanggal.= '<tr><td>'.$row['tanggal'].'</td><td>'.$row['nomor_surat'].'</td><td>'.$row['penerima'].'</td><td>'.$row['pengirim'].'</td><td>'.$row['perihal'].'</td><td>'.$row['keterangan'].'</td></tr>';
			}
...
<html>
<head>
<title>Cetak Arsip Surat Keluar'.$mulai.' s/d '.$akhir.'</title>
...
<div class="container-fluid">
	<div class="row">
		...
		<div class="col-md-12 text-center">
				<span style="font-size:13px;font-family:Arial;"><b><u>ARSIP SURAT KELUAR</u></b></span><br/>
...
$pdf->render();
$pdf->stream("Arsip Surat Masuk ".$mulai." sampai ".$akhir.".pdf", array("Attachment" => false));
