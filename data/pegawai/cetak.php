...
try {
			...
			$tanggal='';
            $query = $db->prepare("SELECT nip, nama, jabatan, perihal, mulai, berakhir, keterangan FROM e_cuti WHERE mulai BETWEEN '$mulai' AND '$akhir' ");
			$query->execute();
			while($row = $query->fetch(PDO::FETCH_BOTH)) {
				$tanggal.= '<tr><td>'.$row['nip'].'</td><td>'.$row['nama'].'</td><td>'.$row['jabatan'].'</td><td>'.$row['perihal'].'</td><td>'.$row['mulai'].'</td><td>'.$row['berakhir'].'</td>'.'</td><td>'.$row['keterangan'].'</td></tr>';
			}
...
<html>
<head>
<title>Cetak Arsip Ijin & Cuti '.$mulai.' s/d '.$akhir.'</title>
...
<div class="container-fluid">
	<div class="row">
		...
		<div class="col-md-12 text-center">
				<span style="font-size:13px;font-family:Arial;"><b><u>ARSIP IJIN & CUTI</u></b></span><br/>
...
$pdf->render();
$pdf->stream("Arsip Ijin & Cuti ".$mulai." sampai ".$akhir.".pdf", array("Attachment" => false));
